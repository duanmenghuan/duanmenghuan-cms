...
<form action="" id="form">
    ...
    <div class="form-group row ">
        <label for="title">文章标题</label> <input type="text"
                                               class="form-control" id="title" value="${article.title}" name="title"
                                               placeholder="请输入标题">
    </div>
    ...
    $(function () {
        ...
        /* $.ajax({
           ...
           success:function(list){
               ...
               for(var i in list){
                   if(
        ${article.channelId}==list[i].id){
					$("#channel").append("<option selected value='"+list[i].id+"'>"+list[i].name+"</option>")
					
					// 频道的回显
					 $("#category").empty();
						//根据ID 获取栏目下的分类
					 $.get("/article/getCatsByChn",{channelId:
        ${article.channelId}},function(catlist){
